...
days = ["15.06","16.06","17.06","18.06","21.06","22.06","23.06","24.06","25.06","29.06","30.06","01.07","02.07"]
logs = {}
for day in days:
    d,m = day.split(".")
    logs[day] = [os.path.basename(f) for f in sorted(glob("../logs-*-%s-%s/notes/*" % (m,d)))]
...
class Note:
    ...
    def __init__(self, raw):
        ...
        self.log = logs[self.date][self.day_id] if self.day_id < len(logs[self.date]) else "written note"

    def __str__(self):
        #return "- **#%s** (%s/%s) [%s]:\n\n%s\n\n``%s``\n\n" % (self.id, self.date, self.day_id, self.code, self.text,self.log)
        return "- **#%s** (%s/%s): %s\n\n" % (self.id, self.date, self.day_id, self.text)



with open("joe_notes",'r') as f:
